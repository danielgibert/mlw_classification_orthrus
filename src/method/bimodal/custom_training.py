...
from src.method.bimodal.base_architecture import BimodalCNN
#from src.method.opcodes_component.base_architecture import ShallowCNN as OpcodesShallowCNN
#from src.method.bytes_component.base_architecture import ShallowCNN as BytesShallowCNN
from src.method.bimodal.tfreader import make_dataset
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Bimodal CNN Model Training')
    ...
    for epoch in range(num_epochs):
        ...
        # Training loop
        for step, (opcodes, bytes, y) in enumerate(d_train):

            loss, y_ = train_loop(opcodes, bytes, y, True)
        ...
        for opcodes_batch_val, bytes_batch_val, y_batch_val in d_val:
            val_logits = model(opcodes_batch_val, bytes_batch_val, False)
    ...
    if args.test_tfrecord is not None:
        ...
        for opcodes_batch_test, bytes_batch_val, y_batch_test in d_test:
            test_logits = model(opcodes_batch_test, bytes_batch_val, False)
